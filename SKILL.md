...
Claude Code 返回的 `result` 字段内容，**必须原样、完整地返回给用户**，不得：
- 自行总结、缩写、提炼要点
- 添加自己的分析或建议
- 只摘取部分内容
- 用自己的话重新表述

**唯一允许的额外内容**：在 result 原文之前加一行简短状态提示（如 `✅ 已发送`），仅此而已。
...
脚本输出 JSON 格式，**必须将 `result` 字段的完整内容原样返回给用户**（参见规则三）。

仅允许在原文前加一行状态提示，如：
- `✅ 已发送`（普通消息）
- `✅ 新会话已创建`（仅 `-new` 无 content）
- `✅ 新会话已创建并发送`（`-new` 带 content）

**禁止对 result 内容做任何总结、缩写、改写或省略。**

上下文用量信息已由脚本**前置**到 `result` 开头，格式如：`📊 claude-opus-4-6: 20% (40000/200000 tokens)`
